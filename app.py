...
if not GENAI_API_KEY:
    ...
    print(f"   I looked for the file here: {env_path}")
else:
    print(f"✅ API Key found (Length: {len(GENAI_API_KEY)})")
...
def kill_process_on_port(port):
    try:
        # Cross-platform check might be needed, this is for Unix-like. 
        # For full robustness, consider psutil in future.
...
def patch_existing_projects():
    print("--- 🔧 Auto-Patching Projects ---")
    for project in os.listdir(PROJECTS_DIR):
        project_path = os.path.join(PROJECTS_DIR, project)
        if not os.path.isdir(project_path): continue
        ensure_aux_files(project_path, project)
...
def generate():
    ...
    try:
        ...
        4. If editing ONE file, you MUST return the others unchanged in the dictionary, OR ensure your code handles partial updates.
...
# --- ROBUST PARSING BLOCK ---
        ...
        try:
            # First try: Standard strict JSON parsing
            result = json.loads(raw_text)
        except json.JSONDecodeError:
            print("⚠️ JSON Error detected. Attempting to sanitize...")
            try:
                # Second try: Fix invalid escapes using Regex
                # This finds backslashes followed by characters that AREN'T valid JSON escapes 
                # (valid: " \ / b f n r t u) and double-escapes them.
                import re
                sanitized = re.sub(r'\\(?![/u"\\bfnrt])', r'\\\\', raw_text)
                result = json.loads(sanitized)
            except Exception as e:
                # Third try: Python's ast.literal_eval (Handles Python-style dicts better)
                # We must convert JSON null/true/false to Python None/True/False for this to work
                try:
        ...
        # 2. Save HTML/CSS (Multi-File Handling)
        raw_html = result.get('html_code')
        
        # Fix stringified dicts
        ...
        if isinstance(raw_html, dict):
            ...
            # Fallback for simple single-file responses
...
# --- PREVIEW ROUTE (REQUIRED FOR EDIT MODE) ---
@app.route('/preview/<project>')
def preview_project(project):
    try:
        path = get_project_dir(project)
        # We serve the raw file content. This allows the builder to "see" what is editable.
        # Note: Complex Flask logic (loops) won't render here, but static text will.
